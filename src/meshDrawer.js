...
void main()
{
    ...
    diff = max(dot(normal, lightdir), 0.0) * 0.6; // Scale diffuse by 0.6 for better balance

    // Calculate specular lighting
    vec3 viewDir = normalize(-vPosition); // Camera is at (0,0,0) in view space
    vec3 reflectDir = reflect(-lightdir, normal);
    spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp) * 0.4; // Scale specular by 0.4
